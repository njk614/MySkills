...
- `thermal_comfort` - 热舒适 (score, L2)
- `humidity_comfort` - 湿度舒适 (score, L2)
- `air_quality_comfort` - 空气质量 (score, L2)
- `acoustic_comfort` - 声舒适 (score, L2)
- `lighting_comfort` - 光照舒适 (score, L2)
- `environmental_comfort` - 环境舒适 (score, L1)
...
- 如果用户只说了指标，没有说明区域，不要默认区域，先追问用户要查询哪个区域。
- 如果用户只说了指标，没有说明时间范围或时间粒度，不要默认查当前值或默认查趋势，先追问用户要看当前、最近 1 小时、最近 24 小时、最近一周，还是其它时间范围。
- 在区域和时间范围都不明确时，不要发起 API 调用，应先让用户补充信息。
- 推荐追问方式："你想看哪个区域的这个指标？主场、大会议室、小会议室、机房还是整层？另外你想看当前值，还是某个时间段的趋势？"
...
3. **缺少区域或时间时必须先澄清**：
   - 只说指标、不说区域时，先追问区域，不要默认主场或其它区域。
   - 只说指标、不说时间时，先追问是查当前值还是查趋势；如果查趋势，再追问时间范围或粒度。
   - 只有在 area_name 和查询类型（最新值或趋势）明确后，才允许构造 API 请求。

4. **默认参数**：
   - metric_level: 默认 L3，也支持 L2、L1
   - floor_no: 20（当前固定值）
   - 区域：支持主场、大会议室、小会议室、机房、整层
   - 当 area_name=整层 时，仅允许使用 L2 或 L1
...
5. **特殊情况**：
   - 如果用户只说了某个指标名称，例如"CO2 达标率怎么样"、"环境舒适多少"，但没有提供区域和时间，不返回猜测结果，先要求用户选择区域和时间范围。
...
默认使用 `http://172.16.1.29:18081`。

如服务部署在其它可达地址，可通过环境变量 `TWINIOC_ENVMETRIC_BASE_URL` 或脚本参数 `--base-url` 覆盖。
...
- 当前数据库中的 metric_level 实际支持 L3、L2、L1；整层仅支持 L2、L1。
- air_quality_index 是空气质量族的综合评分。
- light_uniformity_1h 是比值，不是百分数。
- temp_out 是历史温度统计兼容表。
- 区域名称按文档归一为主场、小会议室、大会议室、机房、整层。
