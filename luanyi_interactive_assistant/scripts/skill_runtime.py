...
8. 当用户输入跟聚焦对象和选中对象相关的问题时，对象名称存在时联系询问的上下文，按以下三种情况严格判断：
   - 情况一：对话开始或没有任何历史指令 → 输出必须包含"A02：层级切换：（对象所在层级）"；
...
5. 示例5（情况三）：上一个对象操作是聚焦传感器10（楼层20），之后执行了"A04：层级切换：上一层"，再聚焦传感器8（楼层20）→ 两次对象操作之间存在A04，必须输出层级切换：[A02：层级切换：楼层20$B01：聚焦对象：传感器8]
"""
