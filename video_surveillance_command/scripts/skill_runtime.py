...
def _expand_instruction_order(instruction_order: str) -> str:
    """将指令串中的无参指令码展开为完整形式，如 E08  E08：视频：下一个视频。"""
...
def _append_history(
    ...
    session.history_user.append({"question": query, "content": raw_bracket})
...
async def get_session_context(token: str, session_id: str = "default") -> dict[str, Any]:
    """获取对话历史，供 Copilot 作为推理上下文使用。"""
    if not token:
