...
>
> 0. 孪易服务基础地址默认使用 `https://www.twinioc.net`；如果调用方显式传入了 `base_url`，则必须使用该地址继续拼接固定路径。
...
14. 当用户与导航路径、去开会、去打印文件相关时，无需查询任何数据，直接识别并输出对应指令：

- 用户想去小会议室、去开会、会议室在哪、指引去小会议室等表达，识别为 `B11：显隐路径：我要去开会`
- 用户想打印文件、找打印机、打印机在哪、指引去打印机等表达，识别为 `B12：显隐路径：我要打印文件`
- 英文场景：用户输入 "go to meeting"、"where is the meeting room"、"navigate to meeting room"、"I want to go to a meeting" 等，同样识别为 `B11：显隐路径：我要去开会`
- 英文场景：用户输入 "print a file"、"where is the printer"、"navigate to printer"、"I need to print" 等，同样识别为 `B12：显隐路径：我要打印文件`
- 这两条指令直接执行，无需用户确认。

15. 当用户说"打开资产盘点"、"开启资产盘点"或英文 "open asset inventory" 等时，识别为 `B13：资产盘点：打开`，直接执行无需确认。
16. 当用户说"关闭资产盘点"、"停止资产盘点"或英文 "close asset inventory" 等时，识别为 `B14：资产盘点：关闭`，直接执行无需确认。
17. 当用户输入 `Show me how CSI works.` 或语义等价的 CSI works 请求时，直接输出 `B15：CSI通感：打开&B01：聚焦对象：Large Meeting Room Area&B07：打开灯：Large Meeting RoomLight Switch`。
18. 当用户输入 `Show me if there is any hidden camera detected` 或语义等价的隐藏摄像头排查请求时，直接输出 `B16：防偷拍监测：打开&B01：聚焦对象：Large Meeting Room Area`。
19. 当用户输入 `I want to book a conference room from 14:00 to 15:00, and reflect it in a chart`、`...view it` 或语义等价的会议室预定并查看图表/视图请求时（时间可变），直接输出 `C01：主题切换：Booking Status`。
20. E34（视频筛选）和 E35（摄像头列表查询）依赖摄像头名称匹配。若通过 `ruisi-twinioc-dataquery-skill get_bind_video_instance_names` 返回的列表中找不到匹配项，输出 `[视频中没有找到匹配的信息]`，不抛出错误。
21. 当用户说"查看XXX摄像头"、"打开XXX视频"时，识别为 E34，筛选参数 = 摄像头名称。
22. 当用户说"XXX摄像头放大/拉近"或"XXX摄像头缩小/拉远"时（含具体摄像头名称），识别为 `E34：筛选：{摄像头名称}&E32：单路云台：拉近` 或 `E34：筛选：{摄像头名称}&E33：单路云台：拉远`；当用户只说"放大、缩小、拉近、拉远、左转、右转、抬头、低头"而不带摄像头名称时，直接输出对应单路云台指令（E28-E33）。
23. 当用户说"查看XXX摄像头告警/事件"时（含具体摄像头名称），识别为 `E34：筛选：{摄像头名称}&E12：事件：事件列表，选中`；当用户只说"看一下告警信息"等不含摄像头名称时，直接输出 `E12：事件：事件列表，选中`（E 系列中告警即事件）。
24. 当用户询问"有哪些摄像头"、"摄像头列表"时，识别为 E35。
25. 区分"上/下一页视频"（E05/E06）和"上/下一个视频"（E08/E09），不可混用。
26. 当用户的问题与视频监控 E 系列功能均不匹配时，输出：`您的提问超出了我能回答的范围，请输入跟视频监控相关的问题！`
...
10. Show me how CSI works.：`[B15：CSI通感：打开&B01：聚焦对象：Large Meeting Room Area&B07：打开灯：Large Meeting RoomLight Switch]`
11. Show me if there is any hidden camera detected：`[B16：防偷拍监测：打开&B01：聚焦对象：Large Meeting Room Area]`
12. I want to book a conference room from 14:00 to 15:00, and reflect it in a chart：`[C01：主题切换：Booking Status]`
