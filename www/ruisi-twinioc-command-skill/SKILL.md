...
17. E34（视频筛选）和 E35（摄像头列表查询）依赖摄像头名称匹配。若通过 `ruisi-twinioc-dataquery-skill get_bind_video_instance_names` 返回的列表中找不到匹配项，输出 `[视频中没有找到匹配的信息]`，不抛出错误。
18. 当用户说"查看XXX摄像头"、"打开XXX视频"时，识别为 E34，筛选参数 = 摄像头名称。
19. 当用户说"XXX摄像头放大/拉近"或"XXX摄像头缩小/拉远"时（含具体摄像头名称），识别为 `E34：筛选：{摄像头名称}&E32：单路云台：拉近` 或 `E34：筛选：{摄像头名称}&E33：单路云台：拉远`；当用户只说"放大、缩小、拉近、拉远、左转、右转、抬头、低头"而不带摄像头名称时，直接输出对应单路云台指令（E28-E33）。
20. 当用户说"查看XXX摄像头告警/事件"时（含具体摄像头名称），识别为 `E34：筛选：{摄像头名称}&E12：事件：事件列表，选中`；当用户只说"看一下告警信息"等不含摄像头名称时，直接输出 `E12：事件：事件列表，选中`（E 系列中告警即事件）。
21. 当用户询问"有哪些摄像头"、"摄像头列表"时，识别为 E35。
22. 区分"上/下一页视频"（E05/E06）和"上/下一个视频"（E08/E09），不可混用。
23. 当用户的问题与视频监控 E 系列功能均不匹配时，输出：`您的提问超出了我能回答的范围，请输入跟视频监控相关的问题！`
